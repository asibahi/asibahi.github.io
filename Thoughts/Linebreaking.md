...
I find my version above easier to grok, to be honest. Either way you organize the code, you are building a graph (or a table), and then you are traversing it to find the shortest path. The main difference is where the cost is encoded. In the graph version, the cost is encoded in the edges themselves, where each edge carries its cost. In the other version, the cost is encoded on each breakpoint, by finding the smallest cost among parent nodes and their encoming edges.
